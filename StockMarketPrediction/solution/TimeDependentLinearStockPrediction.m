...
% Set training, validation and testing (un-normalized) data instances.
[Xps_train,Xps_val,Xps_test,Tps_train,Tps_val,Tps_test] = generate_training_validation_testing_data(Xps,Tps);
...
T = Xps_train(:,1);
% Transposition of training patterns and corresponding targets
% (un-normalized).
P = P';
T = T';
...
% matrix. Each row of matrix R contains a pair of minimum and maximum
% values for the corresponding column of matrix Pnorm. In other words, each
...
% VALIDATION MODE:
% -------------------------------------------------------------------------
% Set validation data patterns and corresponding targets (un-normalized).
P = [Xps_val(:,2:end),Tps_val];
T = Xps_val(:,1);
% Transposition of training patterns and corresponding targets
% (un-normalized).
P = P';
...
% Get network predictions on un-normalized validation data.
Yps_val = sim(net,P);
% Compute the correspodning RMSE value for the un-normalized validation data.
RMSE_val = sqrt(mean((Yps_val-T).^2));
...
Yps_test = sim(net,P);
% Compute the corresponding RMSE value for the un-normalized testing data.
